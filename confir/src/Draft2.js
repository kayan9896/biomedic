...
const Draft2 = ({handleNext, data, handlePoint, handleDrag}) => {
  ...
  useEffect(() => {
    if(changeDown) handlePoint()
      ...
      const sc = (e) => {
        ...
        const nscale = Math.max(0.1, scale + scrolled)
        console.log(nscale, cursor.x, stageRef.current.x(), cursor.x - (cursor.x - stageRef.current.x()) * nscale / scale)
        setTopleft({x: cursor.x - (cursor.x - stageRef.current.x()) * nscale / scale , y: cursor.y - (cursor.y - stageRef.current.y()) * nscale / scale })
        setScale(nscale)
  ...
  useEffect(() => {
    ...
    if (stageRef.current && typeof window.jsPDF !== 'undefined') {
      ...
      const pdf = new window.jsPDF('l', 'px', [w, h]);
    ...
    <>
      ...
      <div id = {'stitch'} ref={containerRef} style={{width: "60vw", height: "80vh", position:"absolute", right: '30vw', top: '5vh', border:'2px solid deepskyblue', overflow:'hidden'}}>
          {/* <InteractiveCanvas
            ...
            setScale={setScale}
          /> */}
        <Stage ref={stageRef} width={containerRef.current?.getBoundingClientRect().width} height={containerRef.current?.getBoundingClientRect().height} position={topleft} scaleX={scale} scaleY={scale} onTouchMove={touchzoom} onTouchEnd={zoomend} onMouseDown={()=>{isStageDown.current = true}} onMouseUp={()=>{isStageDown.current = false}}>
          <Layer>
            {<Image image={stitch}/>}
