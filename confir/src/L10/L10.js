...
function L10({angle,rotationAngle}) {
  ...
  pointer={{
    color: '#ffffff',
    length: 0.1,  // Increased from 0.80 to extend the needle further
    width: 15,
    elastic: false,
    type:'arrow',
    hide:true
...
<GaugeComponent
  ...
  arc={{
    ...
    cornerRadius: 1,
    subArcs: [
      // First OB range up to target1 -0.2
      {
        limit: -50, // -22.5 degrees
        color: 'grey',
        showTick: false,
        tooltip: { text: 'Out' }
      },
...
/>
        ...
        <div style={{position:'absolute', alignItems:'center', top:'666px', left:'496px', zIndex:11}}>
          ...
          <div style={{position:'absolute', top: 0, left:0, width:'100%', textAlign:'center', color:'#CD5445', fontFamily:'abel', fontSize:'80px'}}>{`${angle}°`}</div>
        </div>
        <div style={{position:'absolute', top:'666px', left:'1199px', zIndex:11}}>
          <img src={require('./OBAngleDegreeBg.png')} alt="box" />
          <div style={{position:'absolute', top: 0, left:0, width:'100%', textAlign:'center', color:'#0260A0', fontFamily:'abel', fontSize:'80px'}}>{`${rotationAngle}°`}</div>
        </div>
