...
const Draft2 = ({handleNext, data, handlePoint, handleDrag}) => {
  ...
  const isStageDown = useRef(false);
  ...
  useEffect(() => {
    if(changeDown) handlePoint()
      ...
      const sc = (e) => {
        ...
        const nscale = Math.max(0.1, scale.current + scrolled)
        console.log(nscale, cursor.x, stageRef.current.x(), cursor.x - (cursor.x - stageRef.current.x()) * nscale / scale.current)
        stageRef.current.position({x: cursor.x - (cursor.x - stageRef.current.x()) * nscale / scale.current , y: cursor.y - (cursor.y - stageRef.current.y()) * nscale / scale.current })
        stageRef.current.scale({x: nscale, y: nscale})
    ...
    useEffect(() => {
      const handleGlobalMove = (e) => {
        if (!isStageDown.current) return;
        
        const cursor = stageRef.current.getPointerPosition();
        if (!prevMouse.current) {
          prevMouse.current = cursor
          return
        }
        const dx = cursor.x - prevMouse.current.x;
        const dy = cursor.y - prevMouse.current.y;
    ...
    }, [isStageDown.current, prevMouse.current]);
  ...
  useEffect(() => {
      const handleGlobalMove = (e) => {
        ...
        const x = (cursor.x - stageRef.current.x()) / scale.current;
        const y = (cursor.y - stageRef.current.y()) / scale.current;
        console.log(cursor.x, stageRef.current.x(), scale, x)
        const newp = [...point]
        newp[active] = [x, y]
        setPoint(newp)
        handleDrag(newp);
    ...
    }, [isMouseDown.current, scale.current]);
  ...
  const touchzoom = (e) => {
    ...
    if (touch1 && touch2) {
      ...
      var scale0 = stageRef.current.scaleX() * (dist / lastDist);
      let nscale = Math.max(0.1, Math.min(scale0, 10)); // bounds
      ...
      var newPos = {
          x: newCenter.x - pointTo.x * nscale + dx,
          y: newCenter.y - pointTo.y * nscale + dy,
      };
    ...
    <>
      ...
      <div id = {'stitch'} ref={containerRef} style={{width: "60vw", height: "80vh", position:"absolute", right: '30vw', top: '5vh', border:'2px solid deepskyblue', overflow:'hidden'}}>
          {/* <InteractiveCanvas
            ...
            setScale={setScale}
          /> */}
        <Stage ref={stageRef} width={window.innerWidth} height={window.innerHeight} onTouchMove={touchzoom} onTouchEnd={zoomend} onMouseDown={()=>{isStageDown.current = true}} onMouseUp={()=>{isStageDown.current = false}}>
          <Layer>
            {<Image image={stitch}/>}
